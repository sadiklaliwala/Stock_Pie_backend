...
//        public OrderService(IOrderRepository orderRepo, IWalletService walletService, ITransactionRepository txRepo, IUserRepository userRepo, IAssetRepository assetRepo)
//        {
...
//            return orders.Where(o => o.OrderType == orderType && (string.IsNullOrEmpty(assetSymbol) || o.OrderItem?.Coin?.Id == assetSymbol)).ToList();
//        }

//        public async Task<Order> ProcessOrderAsyncMain(Coin coin, double quantity, OrderType orderType, User user)
//        {
//            var item = new OrderItem { Quantity = quantity, Coin = coin, BuyPrice = (double)coin.CurrentPrice };
//            var order = await CreateOrderAsync(user, item, orderType);

//            if (orderType == OrderType.Buy)
//            {
//                var cost = (decimal)quantity * coin.CurrentPrice;
//                var wallet = await _walletService.PayOrderPaymentAsync(order, user.Id);
...
//                // update/create asset
//                var existing = await _assetRepo.FindByUserAndCoinAsync(user.Id, coin.Id ?? string.Empty);
//                if (existing != null)
//                {
//                    // update average buy price and quantity
//                    var totalQty = existing.Quantity + quantity;
//                    var totalCost = (existing.BuyPrice * existing.Quantity) + ((double)coin.CurrentPrice * quantity);
...
//            {
//                // selling: reduce asset quantity, create tx
//                var existing = await _assetRepo.FindByUserAndCoinAsync(user.Id, coin.Id ?? string.Empty);
//                if (existing == null) throw new InvalidOperationException("Asset not found");
//                if (existing.Quantity < quantity) throw new InvalidOperationException("Insufficient asset quantity to sell");
...
//                // credit user wallet
//                // do not manually change wallet.Balance here; use wallet service to handle transaction record
//                await _walletService.AddBalanceAsync(user.Id, (decimal)quantity * coin.CurrentPrice);
//            }
...
//        {
//            return await ProcessOrderAsync(coin, quantity, OrderType.Buy, user);
//        }

//        public async Task<Order> SellAssest(Coin coin, double quantity, User user)
//        {
//            return await ProcessOrderAsync(coin, quantity, OrderType.Sell, user);
//        }
